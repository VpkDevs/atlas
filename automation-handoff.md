...
3. If key missing → run `credential-acquisition.md` with headed Chromium → let founder sign in/MFA → Atlas resumes, captures or securely prompts for the key, stores env, updates `credentials_index.json`
4. Only if browser-assisted acquisition fails → flag as irreducible → estimate 5 min for human to get key → continue with everything else
...
### Step 0: Credential Acquisition

Before declaring any API setup irreducible, Atlas runs:

```bash
node scripts/atlas/cli.js credentials setup <project-slug> --providers core
```

Atlas opens non-headless Chromium, navigates to each provider credential page, pauses for founder sign-in/MFA, resumes control, captures the API key when the DOM exposes it, falls back to a hidden terminal prompt when it cannot, saves the selected env file, updates `credentials_index.json`, and continues to the next provider.

Any `userMust` for missing credentials must include `browser: credential acquisition attempted` in `layers_attempted`.
...
If key missing: run `atlas credentials setup <project-slug> --providers betteruptime`. Only if that fails, flag irreducible step "Get API key at betteruptime.com ($0 free tier)".
...
If not done: commit config now, run `atlas credentials setup <project-slug> --providers sentry`, then flag DSN as env var to add only if browser-assisted capture fails.
...
None detected → run `atlas credentials setup <project-slug> --providers resend`; if still missing, write sequences as markdown and flag irreducible step (get Resend key — free tier: 3K/mo)
```
