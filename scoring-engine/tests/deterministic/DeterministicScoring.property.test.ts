...
describe('Property-Based Tests: Deterministic Scoring Engine', () => {
  ...
  describe('Property 1: Deterministic Score Computation', () => {
    it('should produce identical results for identical inputs and seed', () => {
      fc.assert(
        fc.property(
          fc.record({
    ...
    it('should produce different results for different seeds', () => {
      fc.assert(
        fc.property(
          ...
          }).filter(({ seed1, seed2 }) => seed1 !== seed2), // Ensure different seeds
          ({ seed1, seed2, data }) => {
            const scoringFunction: PureFunction<typeof data, number> = {
              id: 'test-score-2',
              name: 'Test Score 2',
  ...
  describe('Property 3: Time-Based Score Reproducibility', () => {
    it('should produce identical results with fixed timestamp in evaluation mode', () => {
      fc.assert(
        fc.property(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
    ...
    it('should produce different results with different fixed timestamps', () => {
      fc.assert(
        fc.property(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
  ...
  describe('Property 4: External Data Fallback Determinism', () => {
    it('should return consistent fallback values when external data is unavailable', () => {
      fc.assert(
        fc.property(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
            fallbackValue: fc.float({ min: 0, max: 100 }),
            data: fc.record({
              value: fc.float({ min: 0, max: 100 }),
            }),
          }),
    ...
    it('should handle multiple fallback scenarios deterministically', () => {
      fc.assert(
        fc.property(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
  ...
  describe('Additional Property: Cache Consistency', () => {
    it('should return identical results from cache and fresh computation', () => {
      fc.assert(
        fc.property(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
          ...
          ({ seed, data }) => {
            ...
            const result1 = registry.executeFunction('caching-score', data, context);
            
            // Second call - should return cached result
            const result2 = registry.executeFunction('caching-score', data, context);
