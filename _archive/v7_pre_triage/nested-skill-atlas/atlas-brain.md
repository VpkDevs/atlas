...
Atlas operates across 22 phases (0-21) plus perpetual operator loops spanning days or weeks. Without atlas-brain.md:
...
With atlas-brain.md: `/atlas resume` reads the brain file and picks up in under 60 seconds, with full context.

---

## atlas-brain.md Location and Format

Written to: `~/.atlas/portfolio/[slug]/atlas-brain.md`
...
This protocol is MANDATORY for every Atlas invocation to ensure deterministic resumption and prevent context drift.

1. **Phase 0 Sync:** Atlas MUST read `atlas-brain.md` and `context.json` before performing any tool calls that mutate state or code.
2. **Mental Model Grounding:** After reading, Atlas MUST output the `QUICK RESUME CARD` (see below) to the founder. This serves as the shared "Source of Truth" for the session.
3. **Decision Lock:** Atlas MUST NOT re-derive any decision recorded in the `Key Decisions Made` section. If a decision needs to be changed, the founder must explicitly request an override.
4. **Action Sequence:**
   - Detect project slug.
   - Read state files.
   - Output `QUICK RESUME CARD`.
   - Proceed to the `Next Action` defined in the brain.
...
NON-BLOCKING (do when convenient):
  ...
  → [um-002] Record 2-min demo video — see launch-strategy.md
...
PROCEDURE brain_init:
  ...
  1. Check for existing atlas-brain.md:
     cat ~/.atlas/portfolio/[slug]/atlas-brain.md
  ...
  2. IF brain exists:
     ...
     e. On 'restart': archive brain as `atlas-brain-[date].md.bak`, start fresh
     f. If no response in 60s (autonomous mode): auto-resume
     
  3. IF brain does not exist:
     a. Create ~/.atlas/portfolio/[slug]/ directory
     b. Initialize blank atlas-brain.md from schema above
...
**Key Decisions are permanent.** Once a decision is recorded in atlas-brain.md, Atlas does not re-derive it on subsequent sessions. If the founder wants to change a decision, they edit atlas-brain.md directly and Atlas respects the update.

---

## Brain Read Procedure (Every Phase Start)

Before starting any phase, Atlas reads atlas-brain.md to check:
...
At the end of every phase checkpoint output, Atlas writes to atlas-brain.md before displaying the checkpoint:

```text
[Atlas writes atlas-brain.md]
...
**Autonomous mode** (set in atlas-brain.md `"autonomous": true`): Atlas never pauses. Routes around every blocker using End-Run Protocol. Surfaces all pending items at natural stopping points. Used by founders who want Atlas to execute continuously without confirmation loops.

To enable: `"autonomous": true` in atlas-brain.md `## Settings` section.

---

## Recovery from Corrupted Brain

If atlas-brain.md is corrupted or missing mid-run:

```text
PROCEDURE brain_recovery:
  1. Check for atlas-brain.md.bak (previous write backup)
...
Append to `~/.atlas/portfolio/[slug]/atlas-brain.md` after each session under this heading.
...
Written to the top of every atlas-brain.md update. Printed first on any `/atlas resume`.
...
PROCEDURE brain_diff_write(changed_fields):
  ...
  for field in changed_fields:
    find_section(atlas-brain.md, field)
...
If brain file exceeds 2,000 lines, split into `atlas-brain.md` (live) and `atlas-brain-archive.md` (historical).
...
Archive policy: Move Phase History rows older than 30 days to atlas-brain-archive.md
              Move Learning Log entries older than 60 days to atlas-brain-archive.md
...
- [ ] `~/.atlas/portfolio/[slug]/atlas-brain.md` exists and is valid markdown
...
- ❌ Phase completed but atlas-brain.md not updated
...
- ❌ Resuming a session without reading atlas-brain.md first
