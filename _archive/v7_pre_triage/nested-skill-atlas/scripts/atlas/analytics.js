...
class AnalyticsEngine {
  ...
  generateExecutiveSummary() {
    ...
    const pulse = this.loadPulse();
    const mode = context.governance?.capital_mode || 'UNKNOWN';
    const score = context.scores?.sovereign_score || 0;

    return {
      title: 'Executive Summary',
      content: {
        status: score > 80 ? 'healthy' : score > 60 ? 'stable' : 'critical',
        mode: mode,
        key_highlights: [
          `Sovereign Score is currently ${score.toFixed(1)}`,
          `Capital Operating Mode: ${mode}`,
          pulse ? `Last pulse collected at ${pulse.timestamp}` : 'Waiting for first metrics pulse',
          `Active Project Lane: ${context.context?.lane || 'parked'}`
        ],
        risks: pulse?.anomalies?.map(a => a.message) || ['No metrics data available']
      }
    };
  }
  ...
  generateKeyMetrics() {
    ...
    return {
      ...
      content: {
        revenue: {
          mrr: s?.available ? s.mrr : 0,
          new_mrr_7d: s?.available ? s.new_mrr_7d : 0,
          growth_rate: pulse?.deltas?.mrr || 0,
          trend: (pulse?.deltas?.mrr || 0) > 0 ? 'up' : 'down'
        },
        customers: {
          total: s?.available ? s.customer_count : 0,
          churned_mrr_7d: s?.available ? s.churned_mrr_7d : 0
        },
        system_health: {
          uptime: pulse?.metrics?.uptime?.lowest_uptime_7d || 100,
          unresolved_errors: pulse?.metrics?.sentry?.unresolved_issues || 0
        }
      }
    };
  }
  ...
  loadPulse() {
    const pulsePath = path.join(STATE_DIR, 'last_pulse.json');
