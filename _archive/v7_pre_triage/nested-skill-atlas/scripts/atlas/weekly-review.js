...
async function main() {
  ...
  // Step 3: Auto-Ops (Autonomous Incident & Governance Manager)
  if (fs.existsSync(PULSE_PATH)) {
    const autoOpsArgs = [PULSE_PATH];
    if (fs.existsSync(DECISION_PATH)) autoOpsArgs.push(DECISION_PATH);
    run('AUTO_OPS', 'auto-ops.js', autoOpsArgs);
  }

  // Step 4: Open PR / GitHub Issue
