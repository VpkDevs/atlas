...
Deliverable:
- `docs/founder/fusion-report.md` with routed domains, chosen interventions, expected impact, and rollback notes.

### Subcommands (v8.3 Functional)

| Command | Effect | CLI Equivalent |
|---------|--------|----------------|
| `/atlas` | Start/Resume Sovereign Operation | `node scripts/atlas/cli.js status` |
| `/atlas status` | Detailed phase/score dashboard | `node scripts/atlas/cli.js status --detailed` |
| `/atlas diag` | Run Pulse → Decide diagnostic pipeline | `node scripts/atlas/cli.js diagnose` |
| `/atlas score` | Recompute all deterministic engine scores | `node scripts/atlas/cli.js score calculate` |
| `/atlas predict` | Generate 7-day trend analysis | `node scripts/atlas/cli.js score predict` |
| `/atlas portfolio` | Manage project lanes and lanes | `node scripts/atlas/cli.js portfolio list` |
| `/atlas focus` | Shift primary focus project | `node scripts/atlas/cli.js portfolio focus <name>` |
| `/atlas rebalance` | Audit lanes and trigger Capital Governor | `node scripts/atlas/cli.js portfolio rebalance` |
| `/atlas fusion` | Route task to specialized sub-agent | `node scripts/atlas/cli.js fusion route "<task>"` |
| `/atlas sync` | Omni-Sync all project adapters | `node scripts/atlas/cli.js adapter sync` |
| `/atlas dashboard` | Open the web-based business cockpit | `node scripts/atlas/cli.js dashboard` |
| `/atlas resume` | Resume exactly from the Brain's state | *Mandatory Phase 0 Sync* |
...
PROCEDURE atlas_diag:
  ...
  6. CONTENT ENGINE
     ...
     [ ] Check content-calendar-30.md exists
...
PROCEDURE context_load:
  ...
  3. MANDATORY BRAIN SYNC: Read ~/.atlas/portfolio/[slug]/atlas-brain.md and context.json immediately.
  4. IF context.json exists:
       a. Read it (JSON parse fail → restore from .bak)
      b. Read atlas-brain.md (if exists) to hydrate current phase, next action, and pending blockers
  ...
  5. STATE SUMMARY: You MUST output a concise summary of the current state (Phase, Score, Next Action, Blockers) before any other action.
  6. IF context.json missing:
       a. Initialize from skeleton schema
       b. Set mode = FIRST-RUN
  7. Copy dashboard-template.html → ~/.atlas/dashboard.html
  8. Inject ATLAS_STATE JSON block
  9. Open dashboard (platform-aware: Start-Process / open / xdg-open)
  10. Say: "Dashboard opened. Mode: [mode]. Phase: [phase]. Score: [score]."
```
...
| 5 | **Launch Strategy** | `launch-strategy.md` | `docs/founder/launch-sequence.md` generated; 0 `[TODO]` placeholders |
...
| — | Atlas Brain Module | `atlas-brain.md` | Defines and maintains runtime `atlas-brain.md` session continuity state |
...
│       ├── atlas-brain.md                       ← resumable phase state + decisions + rollback registry
...
│   └── cookie-policy.md
├── founder/
│   ├── runbook.md, launch-sequence.md, launch-timeline.md
│   ├── content-calendar-30.md, seo-strategy.md, press-kit.md
│   ├── marketing-playbook.md, growth-engine.md
│   ├── business-setup.md, startup-credits.md, tax-calendar.md
│   ├── contractor-onboarding.md, operations-handbook.md
│   ├── support-playbook.md, first-dollar-sprint.md
│   ├── exit-readiness.md, data-room/, acquire-listing.md
│   ├── money-engine.md, pricing-lab.md, cashflow.md
│   ├── offer-library.md, funnel-experiments.md
│   ├── roadmap.md, investor-update-template.md
│   └── your-next-action.md
└── api/api.md
...
- ❌ Wrote a `launch-strategy.md`-style memo instead of a numbered `docs/founder/launch-sequence.md` recipe
...
**Atlas v8.3 Sovereign Engine is now active. The execution contract is deterministic. Proceed.**
