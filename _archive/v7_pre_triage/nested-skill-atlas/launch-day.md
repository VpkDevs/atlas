...
Atlas processes `launch-sequence.md` line by line, in order, with **clock-driven dispatching**:

```
For each scheduled item in launch-sequence.md:
...
Pre-drafted body in: docs/founder/launch_assets/hacker-news.md
...
1. Matches against `war-room-responses.md` patterns
...
- [ ] All `launch-sequence.md` items have a final status (executed | skipped | deferred)
...
- ❌ Letting an item in launch-sequence.md fire without verification
- ❌ Auto-firing a response from war-room-responses.md to a context where it doesn't fit
