...
If no support tool: output `docs/founder/support-playbook.md` with 20 scripted responses.
...
BUFFER_ACCESS_TOKEN in .env → YES → Already done in Module 5 (verify scheduled)
                             → NO  → Output content-calendar-30.md
```
...
**Skip entirely if `monetization_status: pre-revenue`.** Log: "Payment automation [LOCKED: pre-revenue] — add Stripe to activate (see business-setup.md)."
...
`docs/founder/contractor-onboarding.md`:
...
Atlas did:
  ...
  ✓ Support: [N] responses scripted in support-playbook.md
