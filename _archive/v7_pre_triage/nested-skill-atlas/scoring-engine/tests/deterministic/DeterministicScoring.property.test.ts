...
function safeFloat(min: number, max: number) {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            data: fc.record({
              value: safeFloat(0, 100),
              weight: safeFloat(0, 1),
          ...
          async ({ seed, data }) => {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('test-score', data, context),
              registry.executeFunction('test-score', data, context),
            ]);
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            data: fc.record({
              value: fc.integer({ min: 1, max: 10_000 }).map((n) => n / 100),
            }),
          }).filter(({ seed1, seed2 }) => seed1 !== seed2),
          async ({ seed1, seed2, data }) => {
            const scoringFunction: PureFunction<typeof data, number> = {
              id: 'test-score-2',
              name: 'Test Score 2',
              fn: (inputs, context) => {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('test-score-2', data, { seed: seed1 }),
              registry.executeFunction('test-score-2', data, { seed: seed2 }),
            ]);
            return r1.output !== r2.output;
          }
        ),
    ...
    it('should produce identical results with fixed timestamp in evaluation mode', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ seed, fixedTimestamp, data }) => {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('time-based-score', data, context),
              registry.executeFunction('time-based-score', data, context),
            ]);
            return r1.output === r2.output;
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            ...
            data: fc.record({
              value: safeFloat(10, 100),
            }),
          }).filter(({ timestamp1, timestamp2 }) => timestamp1.getTime() !== timestamp2.getTime()),
          async ({ seed, timestamp1, timestamp2, data }) => {
            const timeBasedFunction: PureFunction<typeof data, number> = {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('time-based-score-2', data, {
                seed,
                evaluationMode: true,
    ...
    it('should return consistent fallback values when external data is unavailable', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.record({
            seed: fc.integer({ min: 0, max: 1000000 }),
            fallbackValue: safeFloat(0, 100),
            data: fc.record({
              value: safeFloat(0, 100),
            }),
          }),
          async ({ seed, fallbackValue, data }) => {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('external-data-score', data, context),
              registry.executeFunction('external-data-score', data, context),
            ]);
            return r1.output === r2.output;
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ seed, data }) => {
            ...
            const [r1, r2] = await Promise.all([
              registry.executeFunction('multi-fallback-score', data, context),
              registry.executeFunction('multi-fallback-score', data, context),
            ]);
            return r1.output === r2.output;
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ seed, data }) => {
            ...
            const r1 = await registry.executeFunction('caching-score', data, context);
