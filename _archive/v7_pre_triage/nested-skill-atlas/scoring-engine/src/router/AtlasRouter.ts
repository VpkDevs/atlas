...
import { IntelligentRouter, Agent } from './IntelligentRouter';
...
export class AtlasRouter {
  ...
  private v2Router: IntelligentRouter;
  ...
  constructor(config?: Partial<RouterConfig>) {
    this.config = {
      ...
      stateDir: path.join(process.env.HOME || process.env.USERPROFILE || '', '.atlas'),
      ...config,
    };
    this.monitor = new PerformanceMonitor(this.config.stateDir);
    this.v2Router = new IntelligentRouter(this.monitor);
  }
  ...
  async classify(userRequest: string): Promise<TaskClassification> {
    ...
    // Step 1: V2 Intelligent Routing
    const v2Result = await this.v2Router.routeTask(userRequest);

    // Step 2: V1 Static Classification
    ...
    if (!bestMatch) {
      const classification: TaskClassification = {
        ...
        assignedAgent: v2Result.agent,
        routingScore: v2Result.score
      };
