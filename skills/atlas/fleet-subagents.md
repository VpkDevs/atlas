...
Atlas (Coordinator) runs the Oracle Tick (see `mission-intelligence.md`) and delegates tasks to Fleet agents. **A Fleet agent is a real dispatched subagent, not a persona described in prose.** The Coordinator is the main Atlas loop (or the workflow script it authors); agents are dispatched through the harness's orchestration primitives per `leverage-engine.md` §2 — the Workflow tool (`agent()` with a structured-output schema) when available, the Agent/Task tool otherwise. ≥3 independent tasks in a tick MUST fan out concurrently.

Each dispatched agent:

1. **Receives** a scoped prompt built from the 5-part Delegation Prompt Contract (`leverage-engine.md`): ROLE / CONTEXT / TASK / BOUNDS / RETURN
2. **Executes** autonomously within its domain boundaries, using the Six-Layer Hierarchy
3. **Returns** a structured result — never free prose

### Delegation Format

The ROLE line comes from this module's roster. The RETURN schema is mandatory:

```json
{
  "outcome": "success | partial | failure",
  "actions_taken": ["Buffer API POST /updates — 7 posts scheduled"],
  "evidence": [{"proof": "api_response", "pointer": "buffer ids [..] with future timestamps"}],
  "metrics_delta": {"scheduled_posts": "+7"},
  "escalations": []
}
```

Dispatch example (Workflow tool):

```js
agent(`ROLE: Atlas Growth — Distribution Commander. Authority: post/schedule social,
reply to comments. Cannot: change product, pricing, spend money, create accounts.
CONTEXT: [paste target_customer, launch_channels_ranked, calendar excerpt]
TASK: Schedule 7 days of content and post the launch announcement.
Acceptance: 7 posts queued with future timestamps; launch post live.
BOUNDS: $0 budget; founder-approved calendar only; escalate per fleet-subagents.md.
RETURN: the JSON schema exactly.`, {label: 'fleet:growth', schema: FLEET_RETURN})
```
...
## Harness Binding (v0.9)

| Fleet concept | Real primitive |
|---|---|
| Coordinator | Main Atlas loop, or the Workflow script it authors for the tick |
| Fleet agent | `agent()` call (Workflow) or Agent-tool subagent with the 5-part contract |
| Persistent specialist | Named agent + SendMessage continuation across turns |
| Parallel tick | `parallel()` / `pipeline()` fan-out — see canonical patterns in `leverage-engine.md` §2 |
| agent_log.md entry | Written by the Coordinator from each agent's structured RETURN |
| Overnight fleet | Heartbeat tick dispatching bounded agents (`leverage-engine.md` §3) |

Fallback: no orchestration available → execute the roster's tasks serially in domain-precedence order and log `orchestration: unavailable` in evidence.jsonl.
