---
name: skill-hygiene
description: Atlas module — doctrine that prevents the skill from accumulating the meta-bloat that produced v0.7.x's 923 files.
---

# Skill Hygiene — The Anti-Bloat Doctrine

*This module exists because the v0.7.2 Atlas skill, at the moment of v0.8.0 audit, contained 923 markdown files, 14 duplicate uppercase/lowercase filename pairs, three nested copies of SKILL.md (one recursively inside itself), a committed `node_modules/` at the skill root, and a SKILL.md body declaring v0.7.2 while the directory contained references to v0.8.3. None of those problems came from a single bad decision. They came from a thousand tiny "let me add a quick summary file" decisions. This is the doctrine that catches the thousand decisions.*
...
3. `CHARTER.md` — the single version-of-record document (de-versioned filename since v0.9; version lives in `package.json`)
...
Each rule above corresponds to a specific failure observed in the v0.7.2 audit. Each is non-arbitrary.

| Rule | Failure It Prevents |
|---|---|
| One SKILL.md per skill | Triple-nested SKILL.md at `/SKILL.md`, `/atlas/SKILL.md`, `/atlas/atlas/SKILL.md` |
| No `IMPROVEMENT*` / `FIX*` / `SUMMARY*` files | 15+ such files accumulated in v0.7.x, none of which Claude reads when running Atlas |
| No uppercase+lowercase pairs | 14 such pairs in v0.7.x, the result of incomplete rename passes |
| No `node_modules/` at root | v0.7.x had 164 subdirectories of node_modules in the skill, bloating loading and confusing scanning |
| No `v[N]` suffixed copies | v0.7.x had `fusion-router.md` AND `fusion-router-v2.md` AND `MODULE_AUDIT_v0.8.3.md` AND `STRATEGIC_ARCHITECTURE_v0.8.3.md` — Claude cannot tell which is canonical |
...
| `STRATEGIC_ARCHITECTURE_v0.8.3.md` | An edit to SKILL.md and a `CHARTER.md` line item |
...
| `TREMENDOUS_IMPROVEMENTS_V8.1.md` | One `CHANGELOG.md` entry under "v0.8.1" |
...
Atlas v0.8.0 has exactly one changelog file: `CHANGELOG.md`. Format:
...
- New: `CHARTER.md` as single source of version truth
...
## v0.7.2 — 2025-05-15
- New: Scoring engine, fusion router v0.7.2, operator playbook, incident protocol
- Verbose v0.7.x/v0.8.x planning documents are recoverable from git history if an audit needs them.
...
- First H1 MUST be the module's name (not "Atlas v0.8.0 — ..."; just "Module Name — ...")
...
In v0.7.x, the skill directory at one point contained:
- `/SKILL.md`
- `/atlas/SKILL.md`
...
**v0.8.0 Doctor Check 4 specifically catches this.** Additionally:
...
In v0.7.x, the response to "we should add anti-pattern detection" was to create a new file (`adversarial-and-epistemic.md`). In v0.8.0, the response is to add a section to an existing module:
...
Atlas ships with ~45 module files. The soft ceiling is 60. At 60, the kernel pattern starts to break down — Claude cannot route between 60+ modules effectively, and human maintainers cannot hold the structure in mind.
...
| "The skill works fine with the extra files" | Until it doesn't. v0.7.x worked fine until the audit. |
