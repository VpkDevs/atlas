...
<body>
  ...
  async routeTask(task) {
    ...
    let agent = 'atlas-product';
    if (/growth|content|launch|customer|outreach|funnel/.test(normalized)) agent = 'atlas-growth';
    if (/monitor|deploy|incident|uptime|automation|ci/.test(normalized)) agent = 'atlas-ops';
    if (/price|revenue|cash|mrr|billing|churn/.test(normalized)) agent = 'atlas-money';
    console.log(JSON.stringify({ task, agent, confidence: 'medium' }, null, 2));
  }
  ...
  async restoreState(backupFile) {
    ...
    for (const project of backup.projects) {
      if (!project.slug || !project.context) continue;
      writeJsonAtomic(path.join(getProjectPath(slugify(project.slug)), 'context.json'), project.context);
    }
