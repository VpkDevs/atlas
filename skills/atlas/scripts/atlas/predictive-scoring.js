...
function normalizeHistory(history) {
  const normalized = defaultHistory();
  for (const key of Object.keys(normalized)) {
    normalized[key] = Array.isArray(history?.[key]) ? history[key] : [];
  }
  for (const key of ['sovereign_scores', 'revenue_velocity', 'retention_health', 'cash_discipline']) {
    normalized[key] = normalized[key].map(clampScore);
  }
  normalized.timestamps = normalized.timestamps.map(String);
  return normalized;
}

function normalizeScoreData(scoreData) {
  return {
    sovereign_score: clampScore(scoreData?.sovereign_score),
    revenue_velocity: clampScore(scoreData?.revenue_velocity),
    retention_health: clampScore(scoreData?.retention_health),
    cash_discipline: clampScore(scoreData?.cash_discipline)
  };
}
...
class PredictiveScoringEngine {
  ...
  loadHistory() {
    try {
      if (fs.existsSync(this.historyPath)) {
        this.history = normalizeHistory(JSON.parse(fs.readFileSync(this.historyPath, 'utf8')));
      } else {
  ...
  saveHistory() {
    writeJsonAtomic(this.historyPath, this.history);
  }

  addScore(scoreData) {
    const timestamp = new Date().toISOString();
    const normalized = normalizeScoreData(scoreData);
    
    this.history.sovereign_scores.push(normalized.sovereign_score);
    this.history.revenue_velocity.push(normalized.revenue_velocity);
    this.history.retention_health.push(normalized.retention_health);
    this.history.cash_discipline.push(normalized.cash_discipline);
  ...
  predictTrend(scores, daysAhead = 7) {
    const cleanScores = Array.isArray(scores) ? scores.map(clampScore).filter(Number.isFinite) : [];
    const cleanDaysAhead = Math.max(1, Number.parseInt(daysAhead, 10) || 7);
    if (cleanScores.length < 3) return { trend: 'insufficient_data', prediction: null, confidence: 0 };
    ...
    const prediction = intercept + slope * (n + cleanDaysAhead - 1);
    const rSquared = this.calculateRSquared(cleanScores, slope, intercept);
    ...
    return {
      ...
      confidence: rSquared,
      slope,
      days_to_target: this.calculateDaysToTarget(cleanScores, 90) // Days to reach sovereign score 90
    };
  }
  ...
  generateInsights(scoreData) {
    const insights = [];
    const normalizedScoreData = normalizeScoreData(scoreData);
    
    // Add current scores to history for analysis
    this.addScore(normalizedScoreData);
    ...
    return {
      current_scores: normalizedScoreData,
      predictions,
      insights,
      history_summary: {
  ...
  generateReport(scoreData) {
    const normalizedScoreData = normalizeScoreData(scoreData);
    const analysis = this.generateInsights(normalizedScoreData);
    
    const report = `
# Predictive Scoring Report - ${this.projectSlug} - ${new Date().toISOString().split('T')[0]}

## Current Scores
- **Sovereign Score**: ${normalizedScoreData.sovereign_score}/100
- **Revenue Velocity**: ${normalizedScoreData.revenue_velocity}/100
- **Retention Health**: ${normalizedScoreData.retention_health}/100
- **Cash Discipline**: ${normalizedScoreData.cash_discipline}/100
...
if (require.main === module) {
  ...
  if (args.length >= 5) {
    ...
    const scoreData = {
      sovereign_score: args[1],
      revenue_velocity: args[2],
      retention_health: args[3],
      cash_discipline: args[4]
    };
